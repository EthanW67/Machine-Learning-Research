...
checkpoint = torch.load('/home/ys92/EthanSURF/SURF_Research/SURF_Final/cond_diffu_CTs/CAE_savemodels/checkpoint.pth')
...
with torch.no_grad():
    for i, data in tqdm(enumerate(testloader, 0), total=len(testloader), desc="Processing"):
        ...
        if i == iteration_to_save:
            ...
            plt.savefig('/home/ys92/EthanSURF/SURF_Research/SURF_Final/cond_diffu_CTs/CAE_test_figs/gtcttest_{}_original.png'.format(i), dpi=300) # ground truth figure

            plt.imshow(ct_test_slice_64.reshape(64, 64).cpu().numpy(), cmap='gray')
            plt.title('Downsampled CT Slice (64x64)')
            plt.savefig('/home/ys92/EthanSURF/SURF_Research/SURF_Final/cond_diffu_CTs/CAE_test_figs/gtcttest_{}_64.png'.format(i), dpi=300) # downsampled figure

            plt.imshow(ct_test_slice_256.reshape(256, 256).cpu().numpy(), cmap='gray')
            plt.title('Upsampled CT Slice (256x256)')
            plt.savefig('/home/ys92/EthanSURF/SURF_Research/SURF_Final/cond_diffu_CTs/CAE_test_figs/gtcttest_{}_256.png'.format(i), dpi=300) # upsampled figure

            plt.imshow(predicted_ct_slice.reshape(256, 256).cpu().numpy(), cmap='gray')
            plt.title('Predicted CT Slice (256x256)')
            plt.savefig('/home/ys92/EthanSURF/SURF_Research/SURF_Final/cond_diffu_CTs/CAE_test_figs/predictcttest_{}.png'.format(i), dpi=300) # prediction figure
    ...
    ax.axis('off')
    plt.savefig('/home/ys92/EthanSURF/SURF_Research/SURF_Final/cond_diffu_CTs/CAE_test_figs/average_psnr.png', dpi=300)

    fig, ax = plt.subplots()
    ax.text(0.5, 0.5, f'Average SSIM: {avg_ssim:.4f}', fontsize=15, ha='center')
    ax.axis('off')
    plt.savefig('/home/ys92/EthanSURF/SURF_Research/SURF_Final/cond_diffu_CTs/CAE_test_figs/average_ssim.png', dpi=300)
